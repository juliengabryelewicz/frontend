...
<div class="w-full lg:flex p-5" itemscope itemtype="http://schema.org/Article">
	{#if articleList.image}
	  <div class="h-48 lg:w-48 flex-none bg-cover text-center" style="background-image: url('{process.env.PROTOCOL}://{process.env.API_URL}/{articleList.image.url}')" title="{articleList.titre}">
	  </div>
	{:else}
  ...
  <div class="p-4 flex flex-col justify-between leading-normal">
    <div class="mb-8">
      <div class="text-primary font-bold text-xl uppercase font-title"><a rel=prefetch class="no-underline hover:underline text-black post-link" title='{articleList.titre}' href='/blog/post/{articleList.slug}' itemprop="name">{articleList.titre}</a></div>
